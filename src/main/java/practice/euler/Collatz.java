...
public class Collatz {
    ...
    public static int findLongestChain(int limit, Map<Integer, Integer> cache) {
        ...
        int max = Integer.MIN_VALUE;
        for (int i = 1; i <= limit; i++) {
            max = Math.max(max, getChainLengthMemoized(i, cache));
        }
        return max;
    }

    /**
     * bad routine that results in stack overflow pretty soon; avoid at all costs
     */
    private static int memoizeRecur(int i, Map<Integer, Integer> cache) {
...
//        System.out.println("i: " +i);
        ...
        int count = 0;
        if (i % 2 == 0)
            count = 1 + memoizeRecur(i / 2, cache);
        else
            count = 1 + memoizeRecur((3 * i) + 1, cache);
        cache.put(i, count);
        return count;
    }

    private static int getChainLengthMemoized(int n, Map<Integer, Integer> cache) {
        Integer val = cache.get(n);
        if (val != null)
            return val;
        val = getChainLength(n);
        cache.put(n, val);
        return val;
    }
